    ...
    
{
    ...
    pub fn open_with_wal_check<'a, D: DbDir<'a, Error = F::Error, File<'a> = F>>(&mut self, dir: &'a D) -> Result<(), Error<F::Error>> {
    ...
    fn wal_read_write_page_to_file<A: Allocator + Clone>(
        ...
        match &self.wal_file {
            Some(f) => {
                let header = self.cur_header.as_mut().unwrap();
                let _ = self.page_rw.as_ref().ok_or(Error::InitError)?.read_page(FixedPages::Header as u32, buf.as_mut())?;
